...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am ${global.BotName}, a bot modded by Monkey D Luffy to take your WhatsApp usage into next level.
...
case 'ytmp4': {
    ...
    const viddown = await Miku.sendMessage(from , { text: '📥 Downloading Your Video...' }, { quoted: m } )
    await Miku.sendMessage(from, { delete: viddown.key })
    const vidup = await Miku.sendMessage(from , { text: '📤 Uploading Your Video...' }, { quoted: m } )
    const vid = await Miku.sendMessage(m.chat, { video: { url: media.dl_link }, jpegThumbnail:buf, mimetype: 'video/mp4', fileName: `${media.title}.mp4`, caption: `${global.cap}` }, { quoted: m }).catch((err) => reply(mess.error))
    await Miku.sendMessage(from, { delete: vidup.key }) 
}
break
case 'ytmp3': {	    
...
const auddown = await Miku.sendMessage(from , { text: '📥 Downloading Your Song...' }, { quoted: m } )
await Miku.sendMessage(from, { delete: auddown.key })
const audup = await Miku.sendMessage(from , { text: '📤 Uploading Your Song...' }, { quoted: m } )
const aud = await Miku.sendMessage(m.chat, {audio:{url:media.dl_link}, mimetype:"audio/mpeg", fileName: `${media.title}.mp3`}, { quoted: m }) .catch((err) => reply(mess.error))
await Miku.sendMessage(from, { delete: audup.key })               
}
break
...
case 'ytdoc': {
    ...
    const docdown = await Miku.sendMessage(from , { text: '📥 Downloading Your Song...' }, { quoted: m } )                
         tummb = await getBuffer(anu.thumb)
    audio = await getBuffer(anu.audio)  
    await Miku.sendMessage(from, { delete: docdown.key })
    const docup = await Miku.sendMessage(from , { text: '📤 Uploading Your Song...' }, { quoted: m } )      
    const doc = await Miku.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `${anu.title}`}, { quoted : m }).catch((err) => reply(mess.error))
    await Miku.sendMessage(from, { delete: docup.key })
}
break
case 'song': { 
Miku.sendMessage(from, { react: { text: `🎧`, key: m.key }})    
...
I am *Hinata Hyuga*, a bot modded *Monkey D Luffy*.

┌───『 𝓗𝓪𝓷𝓭𝓸𝓾𝓽 』──
│⊶ 𝑼𝒔𝒆 ${prefix}𝒉𝒆𝒍𝒑 𝒕𝒐 𝒔𝒆𝒆 𝒄𝒐𝒎𝒎𝒂𝒏𝒅𝒔
│⊶ Owner: ${global.OwnerName}
│⊶ ${prefix}script for bot information
...
╚════════════╝
 ...
 🎐 Type " *${prefix}help* " to get full command list.
   ...
   Datebayooooo senpai ....active your Kyubie chackra mod `
...
const txt = `hello  ${pushname} are you a ninja ?`
const mikuarray= [
...
case 'rules':
	      replay(' no command in pm ; avoid spam')
	  break
